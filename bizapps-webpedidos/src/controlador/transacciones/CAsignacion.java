...
public class CAsignacion extends CGenerico {
	...
	public void inicializar() throws IOException {
		...
		if (map != null) {
			if (map.get("idProducto") != null) {
				...
				desde = (String) map.get("desde");
				hasta = (String) map.get("hasta");
				...
				lblProducto.setValue(idProducto + " ,   "
						+ item.getDescription());
				...
				if (!desde.equals("")) {
					Date desdeF = traerFech(desde);
					dtbDesde.setValue(desdeF);
				} else
					dtbDesde.setValue(fecha);
				if (!hasta.equals("")) {
					Date desdeF = traerFech(hasta);
					dtbHasta.setValue(desdeF);
				} else
		...
		Botonera botonera = new Botonera() {
			...
			public void guardar() {
				...
				if (validar()) {
					...
					String fechaD = String.valueOf(formatoFechaRara
							.format(fechaDesde));
					String fechaH = String.valueOf(formatoFechaRara
							.format(fechaHasta));
					...
					limpiar();
					salir();
					controlador.recibirLista(catalogo, cupos, idVendedor,
							idMarca, servicioCupo, servicioProducto);
	...
	public boolean validar() {
		if (spnConsumido.getText().compareTo("") == 0
